	...
	if (banned) {
		...
		if(reconnecting) {
			...
			if (joining[ip]) {
				...
				//Inform everyone of the new arrival.
				sendPublicMessage(Type.JOIN, name);
				if (phase != 0) {
					for (i in players) {
					...
					sendPublicMessage(Type.SETSPEC, name);
				}
				if (alts.length > 0) {
					//Inform everyone of the alt.
					sendPublicMessage(Type.HIGHLIGHT, 'Please be aware that ' + name + ' is an alt of ' + gm.grammarList(alts) + '.');
				}
				//Tell the new arrival what phase it is.
				socket.sendMessage(Type.SETPHASE, phase, true, timer.time);
		...
		if (socket.id == mod) {
			...
			switch (prenot) {
				...
				case 'SHOTVET':
					...
					break;
				case 'VETSHOT':
					players[mod].s.sendMessage(Type.SYSTEM, name + ' attacked a visitor.');
					break;
				case 'RB':
		...
		vote: function (name, forced) {
			if (phase != Phase.VOTING) {
				...
				if (player) {
					...
					socket.sendMessage(Type.SYSTEM, '"' + name + '" is not a valid player.');
				}
			}
		},
						...
						if (this.blackmailed && phase != Phase.PREGAME) {
							...
							if (c.length > 2) {
								if (playernames[c[1]]) {
									...
									this.s.sendMessage(Type.SYSTEM, "'" + c[1] + "' is not a valid player.");
								}
							} else {
						...
						if (c.length == 2) {
							if (playernames[c[1]]) {
								...
								if (players[playernames[c[1]]].cleaned) {
									this.s.sendMessage(Type.SYSTEM, c[1] + "'s Last Will will no longer show upon death.");
								} else {
									this.s.sendMessage(Type.SYSTEM, c[1] + "'s Last Will will show upon death.");
								}
							} else if (!isNaN(c[1])) {
								//Get the numbered player.
								var target = getPlayerByNumber(c[1]);
						...
						if (c.length < 2) {
							...
							if (playernames[c[1]]) {
								if (!players[playernames[c[1]]].silenced) {
									this.s.sendMessage(Type.SYSTEM, c[1] + ' is not silenced.');
								} else {
									sendPublicMessage(Type.HIGHLIGHT, c[1] + ' is now unsilenced.');
								...
								this.s.sendMessage(Type.SYSTEM, 'Could not find player ' + c[1] + '!');
							}
						}
					} else {
						...
						if (c.length < 2) {
							...
							if (playernames[c[1]]) {
								if (players[playernames[c[1]]].silenced) {
									...
									this.s.sendMessage(Type.SYSTEM, 'You have silenced ' + c[1] + ' for the phase. You can use /unsilence to unsilence them early.');
							...
							if (c.length == 3) {
								...
								if (!error) {
									if (c[2] == 'guilty' || c[2] == 'g') {
										...
										this.s.sendMessage(Type.SYSTEM, "'" + c[2] + "' is not a valid option.");
										error = true;
									}
								}
							} else {
								...
								for (i in args) {
									if (args[i] != '') {
										...
										if (p && p != -1) {
											...
											this.s.sendMessage(Type.SYSTEM, 'Invalid player: ' + args[i]);
											error = true;
											break;
										}
									}
					...
					if (this.silenced) {
						...
						if (c.length == 2) {
							...
							if (p && p != -1) {
								...
								this.s.sendMessage(Type.SYSTEM, 'Invalid selection: ' + c[1]);
							}
						} else {
				...
				case 'role':
					if (c.length == 1) {
						...
						if (roles.hasRolecard(rolename)) {
							...
							this.s.sendMessage(Type.SYSTEM, "'" + rolename + "' could not be found.");
						}
					}
					break;
				case 'confirm':
						...
						if (this.dev) {
							...
							if (!isNaN(first[0])) {
								...
								if (/\d+\.\d+\.\d+\.\d+/.test(c[1])) {
									...
									this.s.sendMessage(Type.SYSTEM, 'You banned the ip: ' + c[1] + '. Reason: ' + c.slice(2, c.length).join(' '));
								} else {
									this.s.sendMessage(Type.SYSTEM, 'The argument ' + c[1] + ' was not recognized as an ip.');
								}
							} //name
							else {
								if (playernames[c[1]]) {
									...
									this.s.sendMessage(Type.SYSTEM, "The name '" + c[1] + "' could not be found.");
								}
							}
						} else {
							this.s.sendMessage(Type.SYSTEM, 'You do not have the correct permissions to use this command.');
						...
						if (this.dev) {
							if (isNaN(c[1])) {
								...
								if (player) {
									...
									this.s.sendMessage(Type.SYSTEM, "Cannot find player '" + c[1] + "'");
								}
								...
								this.s.sendMessage(Type.SYSTEM, 'Cannot find user number ' + c[1] + '.');
							}
						} else {
							this.s.sendMessage(Type.SYSTEM, 'You do not have the correct permissions to use this command.');
						}
					...
					if (mod == this.s.id) {
						if (c.length > 2) {
							if (playernames[c[1]]) {
								...
								socket.sendMessage(Type.SYSTEM, "'" + c[1] + "' is not a valid player.");
							}
						} else {
